...
MORSE_DRIVER=/home/greearb/btbits/x64_btbits/kernel/morse_driver_ct
...
sudo env PATH=$PATH make -j4 ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- INSTALL_MOD_PATH=$RPI_ROOT modules_install || exit 1
if [[ -d $MORSE_DRIVER ]]; then
        _PWD=$(pwd)
        cd $MORSE_DRIVER
	make -j4 $ARM_ENV MORSE_TRACE_PATH=$MORSE_DRIVER KERNEL_SRC=$_PWD CONFIG_WLAN_VENDOR_MORSE=m CONFIG_MORSE_USB=y CONFIG_MORSE_COUNTRY="US" CONFIG_MORSE_VENDOR_COMMAND=y CONFIG_MORSE_MONITOR=y CONFIG_MORSE_SDIO=y CONFIG_MORSE_SPI=y CONFIG_MORSE_DEBUG_MASK=2 MORSE_TRACE_PATH=$MORSE_DRIVER modules || exit 1
